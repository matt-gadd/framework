...
function mimic(actual: DNode | DNode[], expected: DNode | DNode[]): [DNode[], DNode[]] {
	...
	for (let i = 0; i < expected.length; i++) {
		...
		} else if ((isWNode(actualNode) || isVNode(actualNode)) && (isWNode(expectedNode) || isVNode(expectedNode))) {
			if (typeof expectedNode.properties === 'function') {
				expectedNode.properties = expectedNode.properties(actualNode.properties);
			}
			const [actualChildren, expectedChildren] = mimic(actualNode.children || [], expectedNode.children || []);
...
export function assertRender(actual: DNode | DNode[], expected: DNode | DNode[], message?: string): void {
	let [decoratedActual, decoratedExpected] = mimic(actual, expected);
